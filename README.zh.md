...
| 工具 | SOP 阶段 | 负责什么 |
|---|---|---|
| `novel_init` | 一 策划 | 项目与商业框架——平台、付费/免费、频道、品类、目标读者、变现方式——以及此后所有指标比较所依据的校准中位（`baselines`，近 30 天同品类指标中位），还有写作参数：细纲窗口、开篇门禁章、存稿目标、细纲上限。**篇幅四问**（总字数、单章字数、是否分卷、分几卷）必须问过用户再录：未问到的项由工具列成待问清单每一步重复，总章数由总字数 ÷ 单章字数推导，三者与卷数互不自洽时报警。 |
...
| `novel_write` | 五 连载 | 单章正文与兑现回报：`write`、`read`、`check`。**初稿按目标字数的 150% 写**（去 AI 化与手改会成段删减），成稿再落回 **-5%/+15%**（欠字比超字严重，下限只给 5%）；工具按章节状态选口径核对，并打印两道长度线。发正文时带上 `delivered=[…]`，它会报出细纲哪些字段已兑现、哪些未兑现、哪些细纲本身没写。每次 `write` 与 `check` 还会附上**去 AI 化统计**（段落长度、对话密度、重复句式）——工具只报症状、从不改写正文，改写由你完成后重新发一遍。 |
...
| 界面 | 位置 | 展示内容 |
|---|---|---|
| **看板** | 会话的视图标签行，紧邻 Chat 与 Trajectory | 一眼看全项目。四列就是章节生命周期——待写 / 写作中 / 已修订 / 已完成；每张卡片带章节号、标题、所属分卷、字数（未定稿的列按**初稿目标 = 成稿 ×150%** 比，已修订/已完成列按成稿目标比）、节拍，以及细纲还欠什么（缺章末钩子、未答字段）。看板上方是 SOP 阶段与进入下一阶段还缺什么、已写章数、总字数、人物、世界观、未兑现伏笔、候选书名。 |
